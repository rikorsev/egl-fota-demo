...
uint32_t SX1232Process( void )
{
    ...
    uint16_t flags1 = SX1232GetFlags1( );
    uint16_t flags2 = SX1232GetFlags2( );
    ...
    case RF_STATE_RX_SYNC:
        if( ( flags1 & RF_IRQFLAGS1_PREAMBLEDETECT ) && ( PreambleDetected == false ) )// Preamble
        {
            PreambleDetected = true;
            Preamble2SyncTimer = GET_TICK_COUNT( );
        }
        if( ( flags1 & RF_IRQFLAGS1_SYNCADDRESSMATCH ) && ( PreambleDetected == true ) && ( SyncWordDetected == false ) ) // SyncAddr
        {
    ...
    case RF_STATE_RX_RUNNING:
        ...
        if( flags2 & RF_IRQFLAGS2_FIFOLEVEL ) // FifoLevel
        {
        ...
        if( flags2 & RF_IRQFLAGS2_CRCOK || flags2 & RF_IRQFLAGS2_PAYLOADREADY) // PayloadReady/CrcOk
        {
    ...
    case RF_STATE_TX_READY_WAIT:
        if( flags1 & RF_IRQFLAGS1_TXREADY )    // TxReady
        {
            if( ( SX1232->RegPacketConfig1 & RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE ) == RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE )
            {
    ...
    case RF_STATE_TX_RUNNING:
        if( !(flags2 & RF_IRQFLAGS2_FIFOLEVEL) )    // FifoLevel below thresold
        {  
        ...
        if( flags2 & RF_IRQFLAGS2_PACKETSENT ) // PacketSent
        {
            TxTimeoutTimer = GET_TICK_COUNT( );
