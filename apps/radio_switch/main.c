...
static const size_t size_table[] = { 16, 32, 64, 254 };
...
static void radio_ping(void)
{
    ...
    egl_result_t result = egl_iface_write(RADIO, (void *)buff, &len);
    EGL_ASSERT_CHECK(result == EGL_SUCCESS, RETURN_VOID);

    result = egl_log_buff(SYSLOG, EGL_LOG_LEVEL_INFO, "send",  (void *)buff, len, 8);
    EGL_ASSERT_CHECK(result == EGL_SUCCESS, RETURN_VOID);
}
